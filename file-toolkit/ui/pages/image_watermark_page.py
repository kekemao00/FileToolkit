...
class ImageWatermarkPage(ft.Column):
    ...
    def __init__(self, page: ft.Page) -> None:
        ...
        self._font_size = ft.TextField(value="24", label="字号", width=80, keyboard_type=ft.KeyboardType.NUMBER, border_radius=8)

        self._progress = ProgressCard(on_cancel=self._cancel)
        self._result = ResultCard(on_reset=self._reset)
        self._run_btn = ft.FilledButton("添加水印", icon=ft.Icons.BRANDING_WATERMARK, on_click=self._start, disabled=True)
        ...
        return ft.Container(
            content=ft.Row(
                controls=[
                    ...
                    ft.Container(
                        content=ft.Icon(ft.Icons.BRANDING_WATERMARK, color="#9333ea", size=20),
                        width=40, height=40, bgcolor="#faf5ff", border_radius=10, alignment=ft.Alignment(0, 0),
                    ),
                    ft.Text("添加水印", size=20, weight=ft.FontWeight.W_600, color="#162f50", font_family="Manrope"),
                ],
