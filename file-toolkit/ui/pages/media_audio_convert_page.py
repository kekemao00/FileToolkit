...
class AudioConvertPage(ft.Column):
    ...
    def __init__(self, page: ft.Page) -> None:
        ...
        self._bitrate = ft.Dropdown(
            ...
            width=160, border_radius=8, label="比特率",
        )

        self._progress = ProgressCard(on_cancel=self._cancel)
        self._result = ResultCard(on_reset=self._reset)
        self._run_btn = ft.FilledButton("开始转换", icon=ft.Icons.AUDIOTRACK, on_click=self._start, disabled=True)
        ...
        return ft.Container(
            content=ft.Row(
                controls=[
                    ...
                    ft.Container(
                        content=ft.Icon(ft.Icons.AUDIOTRACK, color="#ea580c", size=20),
                        width=40, height=40, bgcolor="#fff7ed", border_radius=10, alignment=ft.Alignment(0, 0),
                    ),
                    ft.Text("音频格式转换", size=20, weight=ft.FontWeight.W_600, color="#162f50", font_family="Manrope"),
                ],
